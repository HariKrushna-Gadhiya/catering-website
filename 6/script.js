document.addEventListener('DOMContentLoaded', () => {
    ...
    // 4.5 Order Form Submission via EmailJS
    ...
    if (orderForm) {
        orderForm.addEventListener('submit', async (e) => {
            ...
            const checkedItems = document.querySelectorAll('input[name="menu_items"]:checked');
            const selectedMenu = Array.from(checkedItems).map(cb => `- ${cb.value}`).join('\n');
            
            // Combine message and menu items with plain text newlines
            const combinedMessage = `Additional Requirements:\n${message || 'None'}\n\nSelected Menu Items:\n${selectedMenu || 'None'}`;
            ...
            try {
                const templateParams = {
                    subject: 'New Catering Order Request',
                    name: name,
                    email: email,
                    phone: phone,
                    date: date,
